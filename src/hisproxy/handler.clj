(ns hisproxy.handler
  (:require [compojure.core :refer [defroutes routes wrap-routes]]
            [hisproxy.routes.home :refer [home-routes]]
            
            [hisproxy.middleware :as middleware]
            [hisproxy.session :as session]
...
(defn init
  ...
  (timbre/set-config!
    ...
    {:path "hisproxy.log" :max-size (* 512 1024) :backlog 10})

  (if (env :dev) (parser/cache-off!))
  (start-nrepl)
  ;;start the expired session cleanup job
  (session/start-cleanup-job!)
  (timbre/info "\n-=[ hisproxy started successfully"
...
(defn destroy
  ...
  []
  (timbre/info "hisproxy is shutting down...")
  (stop-nrepl)
